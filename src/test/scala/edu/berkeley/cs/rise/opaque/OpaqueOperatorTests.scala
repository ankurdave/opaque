...
trait OpaqueOperatorTests extends FunSuite with BeforeAndAfterAll { self =>
  ...
  override def beforeAll(): Unit = {
    LogManager.getLogger("edu.berkeley.cs.rise.opaque").setLevel(Level.INFO)
  ...
  testAgainstSpark("triangle counting") { securityLevel =>
    TriangleCounting.run(spark, securityLevel, numPartitions)
  }
